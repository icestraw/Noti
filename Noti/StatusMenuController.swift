...
class StatusMenuController: NSObject, NSUserNotificationCenterDelegate {
    ...
    override func awakeFromNib() {
        ...
        if let button = statusItem.button {
            button.image = NSImage(named: NSImage.Name("StatusBarButtonImageFail"))
    ...
    @objc func stateChange(_ notification: Notification) {
        ...
        if let info = notification.object as? [String: AnyObject] {
            ...
            if let disabled = info["disabled"] as? Bool {
                statusItem.button?.image = NSImage(named: NSImage.Name(disabled ? "StatusBarButtonImageFail" : "StatusBarButtonImage"))
